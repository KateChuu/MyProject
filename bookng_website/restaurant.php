...
<body>
	...
	<?php }else {?>
		...
		<div class="info">
			<h1 style="margin-top: 30px;"><strong><?php echo $row["rName"]; ?></strong></h1>
			<h3 style="margin-top: 5px;">評分: <?php echo $row["rate"]; ?></h3>
			
		<?php
			// 檢查會員是否預約過該餐廳，預約過才能評分
			if(isset($_SESSION["category"]) && $_SESSION["category"] == 'M') {

				// 是會員才能讀取aNo，再用aNo查reservation record
				$sql = "SELECT * FROM `reservationrecord` WHERE aNo = '".$_SESSION["aNo"]."' AND rId = '".$_SESSION["rId"]."'";
			...
			<h4>電話: <?php echo $row["rPhone"]; ?></h4>
			<h4>地址: <?php echo $row["rAddress"]; ?></h4>
			<h4>均消: <?php echo $row["bottomPrice"]; ?></h4>
	...
	<section class="info">
		<h1><strong>評論</strong></h1>
					
	<?php
		if(isset($_SESSION["category"]) && $_SESSION["category"] == 'M') {

			// 檢查會員是否預約過該餐廳，預約過才能評論
        	$sql = "SELECT * FROM `reservationrecord` WHERE aNo = '".$_SESSION["aNo"]."' AND rId = '".$_SESSION["rId"]."'";
			$result = mysqli_query($db_link, $sql);
			if(mysqli_num_rows($result) > 0) { ?>
				...
				<form action="comments.php" method="post">
					<textarea name="text" rows="3" cols="80" placeholder="請輸入評論"></textarea><br>
					<input type="submit" name="add" class="button1" value="提交" style="weight:20px;"><br><br>
				</form>
			<?php }}

			$result = mysqli_query($db_link, "SELECT * FROM comments WHERE rId=" . $_GET['rId'] . "");
			if(mysqli_num_rows($result) > 0) {
				while($row = mysqli_fetch_array($result)) { ?>
					<div class="comment-box">
						<hr style="border-top: 1px solid #395159;width=70%;">
						<h3><?php 
							echo $row["aNo"]; 
							echo str_repeat("&nbsp", 5);
							$sql = "SELECT * FROM `rating` WHERE aNo = '".$row["aNo"]."' AND rId=" . $_GET['rId'] . "";
							$rateResult = mysqli_query($db_link, $sql);
							$rateRow = mysqli_fetch_array($rateResult);
							if(mysqli_num_rows($rateResult) > 0) {
								echo $rateRow['rate'], " ";
								for ($i=0; $i < $rateRow['rate'] - 1; $i++) {
									?><i class="fas fa-star"></i><?php 
								}
								if($rateRow['rate'] - $i > 0.5) { // 0.6-1
									?><i class="fas fa-star"></i><?php
								}else if($rateRow['rate'] - $i != 0) { // 0.1-0.5
